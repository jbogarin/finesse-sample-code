 ...
 */
finesse.clientservices.MasterTunnel = function (host) {
...
finesse.clientservices.ClientServices = (function () {
    ...
    _becomeMaster = function () {
        var tunnel = new finesse.clientservices.MasterTunnel(_config.host),
        publisher = new finesse.clientservices.MasterPublisher(tunnel);
        tunnel.init(_config.id, _config.password, _config.xmppDomain, _config.pubsubDomain);
        _isMaster = true;
    },
...
finesse.restservices.Notifier = finesse.FinesseBase.extend({
	 ...
	 */
	addListener : function (callback_function) {
		this._listenerCallback.push(this._isAFunction(callback_function));
	},
	...
	notifyListeners : function (obj) {
		var len = this._listenerCallback.length, i, callbackFunction;

		for (i = 0; i < len; i += 1) {
			// Be sure that one bad callback does not prevent other listeners
			// from receiving.
			try {
				callbackFunction = this._listenerCallback[i];
				if (typeof callbackFunction === 'function') {
					callbackFunction(obj);
				}
			} catch (err) {
				finesse.utilities.Logger.log("Exception caught: " + err);
			}
		}
	},
	...
	getListeners : function () {
		...
		for (i = 0; i < len; i += 1) {
			changeListenerCopy.push(this._listenerCallback[i]);
		}
...
finesse.restservices.RestBase = finesse.FinesseBase.extend({
    ...
    init: function (options, callbacks, restObj) {
        ...
        this._logger = finesse.utilities.Logger;
        ...
        try {
            ...
            if (requestId && this._pendingCallbacks[requestId]) {
                ...
                if (updateObj.apiErrors && updateObj.apiErrors.apiError) { //Error case
                    ...
                    fakeResponse.object.ApiErrors.ApiError.ErrorData = receivedError.errorData ? receivedError.errorData : undefined;
                    fakeResponse.object.ApiErrors.ApiError.ErrorMessage = receivedError.errorMessage ? receivedError.errorMessage : undefined;
                    fakeResponse.object.ApiErrors.ApiError.ErrorType = receivedError.errorType ? receivedError.errorType : undefined;
        ...
        return function (rsp) {
            ...
            if (options.success || options.error) {
                ...
                if (rspObj.status !== 202 && rsp.headers && rsp.headers.requestid) {
                    ...
                    if (parentUser._pendingCallbacks[requestId]) {
                        delete parentUser._pendingCallbacks[requestId];
                    }
                }
            }
        };
    ...
    */
    restRequest: function (url, options) {
        ...
        if (!options.method || options.method === "GET") {
            ...
            this._pendingCallbacks[uuid].success = options.success;
            this._pendingCallbacks[uuid].error = options.error;
        }
        ...
        finesse.clientservices.ClientServices.makeRequest(encodeURI(url), this._createAjaxHandler(options), params);
    },
...
finesse.restservices.RestCollectionBase = finesse.restservices.RestBase.extend({
    ...
    _addHandlerCb: function (scope) {
        return function (restItemObj) {
            var object = restItemObj.getData(),
            objectId = scope._extractId(object),
    ...
    _deleteHandlerCb: function (scope) {
        return function (restItemObj) {
            var objectId = scope._extractId(restItemObj.getData()),
...
finesse.restservices.User = finesse.restservices.RestBase.extend(/** @lends finesse.restservices.User.prototype */{
    ...
    * @private
     ...
     */
    isOnHoldOrTalking: function () {
        var state = this.getState();
        return state && ((state === finesse.restservices.User.States.TALKING) || (state === finesse.restservices.User.States.HOLD));
    },
    ...
    {
        var self = this, contentBody = {}, result = [], url, reasonCodes, i, reasonCodeArray;
...
finesse.restservices.WrapUpReasons = finesse.restservices.RestCollectionBase.extend(/** @lends finesse.restservices.WrapUpReasons.prototype */{
	...
	/**
     * @private
	 * Gets the REST type for the objects that make up the collection - this is "WrapUpReasons".
	 */
	getRestItemType: function () {
		return "WrapUpReason";
	},
